	...
	function game(Account){
		...
		socket.on('Bag_client',function(data){
			...
			connection.query(SelectGoods,function(err,result){
				if(err){
					console.log('[SelectGoods3 err]-',err.message);
					return;
				}
		...
		socket.on('LevelStar_client',function(data){
			var SelectLevelStar='select level_id,star from user_level where account='+Account;
			connection.query(SelectLevelStar,function(err,result){
				if(err){
					console.log('[SelectLevelStar err]-',err.message);
					return;
				}
				socket.emit('LevelStar_server',{message:result});
			});
		});
		...
		socket.on('battle_client',function(data){
			...
			var temp=(data.level).replace(/[^0-9]/ig,"");
			var LevelId=parseInt((temp[0]-1)*3)+parseInt(temp[1]);
			...
			var SelectStar='select star from user_level where level_id='+mysql.escape(data.level)+'and account='+Account;
			var AddLevel='insert into user_level(account,level_id,star) values(?,?,?)';
			var UpdateStar='update user_level set star=?where level_id='+mysql.escape(data.level)+'and account='+Account;
			...
			var UpdateGoods='update bag set goods_count=goods_count+'+num+' where account='+Account+'and goods_id='+goods;
			...
			//如果能进入该关卡并且胜利
				...
				if(data.level=='1_1'){
					...
					connection.query(SelectHero,function(err,result){
						...
						if(result[0].hero1=='007'){
							star2=1;
						}
						else if(result[0].hero2=='007'){
							star2=1
						}
						else if(result[0].hero3=='007'){
							star2=1;
						}
				...
				if(data.level=='2_1'){
					...
					connection.query(SelectHero,function(err,result){
						...
						if(result[0].hero1=='002'){
							star2=1;
						}
						else if(result[0].hero2=='002'){
							star2=1
						}
						else if(result[0].hero3=='002'){
							star2=1;
						}
				...
				if(data.level=='3_1'){
					...
					connection.query(SelectHero,function(err,result){
						...
						if(result[0].hero1=='001'){
							star2=1;
						}
						else if(result[0].hero2=='001'){
							star2=1
						}
						else if(result[0].hero3=='001'){
							star2=1;
						}
				...
				connection.query(SelectLevel,function(err,result){
					...
					if(result==''){
						...
						connection.query(SelectGoods,function(err,result){
							if(err){
								console.log('[SelectGoods2 err]-',err.message);
								return;
							}
						...
						//把首通获得星级记录入数据库
						var addl=[Account,data.level,star];
				...
				function levelsql(goods,Account){
					//第13关无掉落
					if(LevelId<13){
						//var SelectGoods='select goods_count from bag where account='+Account+'and goods_id='+goods;
						var SelectGoods = "select goods_count from bag where goods_id='"+goods+"' and account='"+Account+"'";
						connection.query(SelectGoods,function(err,result){
							if(err){
								console.log('[SelectGoods1 err]-',err.message);
								return;
							}
							if(result[0].goods_count>0){
								num=1;
				...
				level(Account);
				//固定掉落奖励,同时更新数据库背包中
				function FixedDrop(Account){
					var goods=666;
					var SelectGoods = "select goods_count from bag where goods_id='"+goods+"' and account='"+Account+"'";
					connection.query(SelectGoods,function(err,result){
						if(err){
							console.log('[SelectGoods6 err]-',err.message);
							return;
						}
						if(result[0].goods_count==''){
							num=ArrayBook[LevelId];
							var addg=[Account,goods,num];
							connection.query(AddGoods,addg,function(err,result){
								if(err){
									colsole.log('[AddGoods err]-',err.message);
					...
					arraynum.push(ArrayBook[LevelId]);
				}
				//FixedDrop(Account);
				arraygoods.push(666);//掉落经验书
				arraynum.push(ArrayBook[LevelId]);
				...
				if(star==1){
					exp1+=LevelExp[LevelId][1];
					coin1+=LevelCoin[LevelId][1];
					socket.emit('battle_server',{goods1:arraygoods,goods2:arraynum,star:star,exp:exp1,coin:coin1});
				}
				else if(star==2){
					exp1+=LevelExp[LevelId][2];
					coin1+=LevelCoin[LevelId][2];
					socket.emit('battle_server',{goods1:arraygoods,goods2:arraynum,star:star,exp:exp1,coin:coin1});
				}
				else if(star==3){
					exp1+=LevelExp[LevelId][3];
					coin1+=LevelCoin[LevelId][3];
					socket.emit('battle_server',{goods1:arraygoods,goods2:arraynum,star:star,exp:exp1,coin:coin1});
				}
				var exp_temp=0
				//查询玩家战斗前等级
				...
				connection.query(UpdateExp,exp_temp,function(err,result){
					if(err){
				...
				connection.query(UpdateGrade,UserGrade,function(err,result){
					if(err){
